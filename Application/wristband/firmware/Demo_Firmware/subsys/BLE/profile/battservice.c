...
static uint16 battMaxLevel = BATT_ADC_LEVEL_3V3; // For VDD/3 measurements

// Critical battery level setting
static uint8 battCriticalLevel;

// ADC channel to be used for reading
static uint8 battServiceAdcCh = 0;
...
bStatus_t Batt_AddService( void )
{
  ...
  linkDB_Register( Batt_HandleConnStatusCB );
...
static uint8 battMeasure( void )
{
   ...
   *
   * For a coin cell battery 3.3v = 100%.  The minimum operating
   *
   ...
   * 2.0v = 2974 ADC
   *
   * We need to map ADC values from 4242-2974 to 100%-0%.
   *
   ...
   *
   * percent/adc = 100/1268 = 25/317
   *
   * Resulting in the final equation, with round:
   *
   * percent = (((adc - 2974) * 25) + 316) / 317
   */
  ...
  adc = HalAdcRead( battServiceAdcCh, ADC_PGA_1_2 );
