...
unsigned char ChkSum( PX_FAT_DIR_INFO pDir1 )
{
  ...
  for ( FcbNameLen = 0; FcbNameLen != 11; FcbNameLen++ )
  {
    ...
    Sum = ( ( Sum & 1 ) ?
        0x80 : 0 )
          + ( Sum >> 1 ) + pDir1->DIR_Name[FcbNameLen];
  }
  return ( Sum );
}
...
UINT8 CreatLongName()
{
  ...
  { /* 成功获取上级目录的起始簇号 */
    ...
    { /*下面是读取并分析目录项*/
      pDirName = FBuf ?
          ( PX_FAT_DIR_INFO ) DATA_BASE_BUF1 : ( PX_FAT_DIR_INFO ) DATA_BASE_BUF0; /*短文件名指针指向缓冲区*/
      pLDirName = FBuf ?
          ( P_LONG_NAME ) DATA_BASE_BUF0 : ( P_LONG_NAME ) DATA_BASE_BUF1;
      mCmdParam.Read.mSectorCount = 1; /*读取一扇区数据*/
      mCmdParam.Read.mDataBuffer = FBuf ?
          DATA_BASE_BUF1 : DATA_BASE_BUF0; /*当前处理的文件缓冲区,这里使用双向缓冲区，去处理文件名*/
      FBuf = !FBuf; /*缓冲区标志翻转*/
      i = CHRV3FileRead();
    ...
    if ( FoundNewDev || s == ERR_USB_CONNECT )                         // 有新的USB设备插入
    {
      ...
      if ( s == ERR_SUCCESS )
      {
        ...
        if ( CHRV3DiskStatus >= DISK_MOUNTED )
        {
          ...
          strcpy( ( PCHAR ) mCmdParam.Create.mPathName, "/TCBD~1.CSV" ); /* 新文件名,在根目录下,中文文件名 */
