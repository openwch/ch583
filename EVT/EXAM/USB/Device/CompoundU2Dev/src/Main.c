...
const uint8_t MyCfgDescr[] = {
    0x09, 0x02, 0x3b, 0x00, USB_INTERFACE_MAX_NUM, 0x01, 0x00, 0xA0, 0x32, //配置描述符
                ...
                case UIS_TOKEN_IN:
                {
                    switch(SetupReqCode)
                    {
                        case USB_GET_DESCRIPTOR:
                            ...
                            memcpy(pU2EP0_DataBuf, pDescr, len);                        /* 加载上传数据 */
                            SetupReqLen -= len;
                            pDescr += len;
                            R8_U2EP0_T_LEN = len;
...
{
    HIDMouse[0] = mouse;
    memcpy(pU2EP2_IN_DataBuf, HIDMouse, sizeof(HIDMouse));
    U2DevEP2_IN_Deal(sizeof(HIDMouse));
}
...
{
    HIDKey[2] = key;
    memcpy(pU2EP1_IN_DataBuf, HIDKey, sizeof(HIDKey));
    U2DevEP1_IN_Deal(sizeof(HIDKey));
}
...
int main()
{
    ...
    while(1)
    {
        mDelaymS(1000);
        //鼠标左键
        U2DevHIDMouseReport(0x01);
        mDelaymS(100);
        U2DevHIDMouseReport(0x00);
        mDelaymS(200);

        //键盘按键“wch”
        mDelaymS(1000);
        U2DevHIDKeyReport(0x1A);
        mDelaymS(100);
        U2DevHIDKeyReport(0x00);
        mDelaymS(200);
        U2DevHIDKeyReport(0x06);
        mDelaymS(100);
        U2DevHIDKeyReport(0x00);
        mDelaymS(200);
        U2DevHIDKeyReport(0x0B);
        mDelaymS(100);
        U2DevHIDKeyReport(0x00);
    }
}
