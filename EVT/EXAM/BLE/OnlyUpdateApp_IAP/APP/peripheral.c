...
// GAP GATT Attributes
static uint8 attDeviceName[GAP_DEVICE_NAME_LEN] = "OTAOTA_OTAOTA_OTA";
...
void Rec_OTA_IAP_DataDeal(void)
{
    switch(iap_rec_data.other.buf[0])
    {
		...
		case CMD_IAP_PROM:
		{
			...
			/* 当前是ImageA，直接编程 */
      status = FLASH_ROM_WRITE(OpAdd, iap_rec_data.program.buf, (UINT16) OpParaDataLen);
      OTA_IAP_SendCMDDealSta(status);
			break;
		}
		/* 擦除 -- 蓝牙擦除由主机控制 */
      ...
      SYS_ResetExecute();
		...
		case CMD_IAP_INFO:
		{
			...
			/* IMAGE_B_START_ADD */
			send_buf[1] = (UINT8)(IMAGE_B_START_ADD & 0xff);
			send_buf[2] = (UINT8)((IMAGE_B_START_ADD>>8) & 0xff);
			send_buf[3] = (UINT8)((IMAGE_B_START_ADD>>16) & 0xff);
			send_buf[4] = (UINT8)((IMAGE_B_START_ADD>>24) & 0xff);
