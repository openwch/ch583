/********************************** (C) COPYRIGHT *******************************
 * File Name          : ble_iochub_service_same_16bit_char.c
 * Author             : WCH
 * Version            : V1.1
 * Date               : 2022/01/19
 * Description        :
...
static uint8_t ble_iochub_TxCharProps = GATT_PROP_WRITE_NO_RSP | GATT_PROP_NOTIFY | GATT_PROP_READ;
                                     ...
                                     uint8_t *pValue, uint16_t *pLen, uint16_t offset, uint16_t maxLen, uint8_t method)
{
    ...
    {
        if(!tmos_memcmp(pAttr->type.uuid, ble_iochub_TxCharUUID, ATT_BT_UUID_SIZE))
        {
            *pLen = 1;
            pValue[0] = '1';
        }
