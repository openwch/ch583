/*!
 * \file      ble_uart_service_same_16bit_char.c
 *
 * \brief     ble <--> uart implementation
 *
 * \copyright (C)2002-2021  WCH
 *
 * \code
 *
 *
...
// ble_uart GATT Profile Service UUID
CONST uint8 ble_uart_ServiceUUID[ATT_BT_UUID_SIZE] =
{0xf0,0xff};
...
// Characteristic tx uuid
CONST uint8 ble_uart_TxCharUUID[ATT_BT_UUID_SIZE] =
{0xf1,0xff};
...
static uint8 ble_uart_TxCharProps = GATT_PROP_WRITE_NO_RSP|GATT_PROP_NOTIFY|GATT_PROP_READ;
                                     ...
                                     uint8 *pValue, uint16 *pLen, uint16 offset, uint16 maxLen,uint8 method )
{
    ...
    {
        if(!tmos_memcmp(pAttr->type.uuid, ble_uart_TxCharUUID, ATT_BT_UUID_SIZE))
        {
            *pLen = 1;
            pValue[0] = '1';
        }
